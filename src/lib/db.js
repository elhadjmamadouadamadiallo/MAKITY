...
export async function fetchAnnonces() {
  const { data, error } = await supabase
    .from("annonces")
    .select("id, title, category_slug, price, city, contact_phone, views, status, description, annonceurs(shop_name), photos(url, position)")
    .eq("status", "active")
    .order("id", { ascending: false });
  if (error) throw error;
  return (data || []).map((a) => ({
    id: a.id, title: a.title, cat: a.category_slug, price: a.price, city: a.city,
    shop: a.annonceurs?.shop_name || "Annonceur", phone: a.contact_phone, views: a.views, desc: a.description,
